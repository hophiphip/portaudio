...
output_prefix="inc-"
portaudio_source_folder="portaudio"

# Exit scripts folder and go to source folder
cd ..

# Clean previous build
rm $output_prefix*.go

cd $portaudio_source_folder

genfile() {
	local file="$1"
	local COND="$2"
	local FN=`echo "$output_prefix-$file" | sed 's#/\{1,\}#-#g;s/_/-/g'`
	FN="../${FN/.c/}.go"
...
# remove this.
rm -f ../${output_prefix}src-hostapi-coreaudio-pa-mac-core-old.go